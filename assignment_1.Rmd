...
bike_seoul <-
  bike_seoul %>% select(-starts_with("Visibility"),-starts_with("Dew"),
                        -starts_with("Solar"),-starts_with("Rain"),-starts_with("Snow")) %>%
  ...
  mutate(Holiday = factor(Holiday,levels = c('Holiday', 'No Holiday'),labels = c('Yes', 'No'))) %>% #Holiday factor level changed to Yes/No
...
ggplot(bike_seoul)+geom_point(aes(x=Holiday,y=Count,colour=Holiday))+
...
ggplot(bike_seoul)+geom_point(aes(x=Hour,y=Count,colour=Temperature))+
ggtitle("Plot for hourly rented bike count: Seoul")
#The peak time for renting bike is shown around 8 am and 2-9 pm.
ggplot(bike_seoul)+geom_point(aes(x=Temperature,y=Count))+stat_smooth(aes(x=Temperature,y=Count),method="lm")+
  ggtitle("Plot for Temperature vs Rented bikes: Seoul")
#Shows increase in bikes rented as temperature rises.
ggplot(bike_seoul)+geom_point(aes(x=WindSpeed,y=Count))+
...
ggplot(bike_seoul)+geom_point(aes(x=Humidity,y=Count))+stat_smooth(aes(x=Humidity,y=Count),method = "lm")+
...
#Temperatures of different seasons seperated.
ggplot(bike_washington)+geom_point(aes(x=Holiday,y=Count,colour=Holiday))+
  ggtitle("Plot for Holiday/Non-holiday bike count: Washington")
#Similar to Seoul, Holiday and No holiday bike rented count shows more bikes rented on working days.
...
bike_seoul$LoggedCount<-log10(bike_seoul$Count)
...
summary(seoul_model)
#It shows that the predictors Autumn, Winter, Temperature, Humidity and windspeed, have significance.
...
#they are reliable as sample size is large 8465.
...
#would at least rent 1.97 bikes and at maximum 3.2 bikes.
...
#Humidity and Wind speed all have significance, rejecting the null hypothesis of them being zero.

confint(washington_model,level = 0.97)
#As we have quite a large sample size of 17379, the confidence intervals for the co-efficients are reliable. 

predict(washington_model,newdata = df_pred)
#1.58 bikes rented in Washington

predict(washington_model,newdata = df_pred,interval = "prediction",level = 0.90)
#Therefore, we are (90%) confident with the given parameters,
#would at least rent 0.68 bikes and at maximum 2.48 bikes.
